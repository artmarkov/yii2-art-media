...
class Media extends ActiveRecord implements OwnerAccess
{
    ...
    public function attributeLabels()
    {
        return [
            'id' => Yii::t('art', 'ID'),
            'album_id' => Yii::t('art/media', 'Album'),
            'filename' => Yii::t('art/media', 'Filename'),
            'type' => Yii::t('art', 'Type'),
            'url' => Yii::t('art', 'URL'),
            'title' => Yii::t('art', 'Title'),
            'alt' => Yii::t('art/media', 'Alt Text'),
            'size' => Yii::t('art', 'Size'),
            'description' => Yii::t('art', 'Description'),
            'thumbs' => Yii::t('art/media', 'Thumbnails'),
            'created_at' => Yii::t('art', 'Uploaded'),
            'updated_at' => Yii::t('art', 'Updated'),
            'created_by' => Yii::t('art/media', 'Uploaded By'),
            'updated_by' => Yii::t('art/media', 'Updated By'),
        ];
    }
    ...
    public function saveUploadedFile(array $routes, $rename = false, $allowedFileTypes = null)
    {
        ...
        if (!empty($allowedFileTypes) && is_array($allowedFileTypes) && !in_array($this->file->type, $allowedFileTypes)) {
            throw new \Exception(Yii::t('art/media', 'Sorry, [{filetype}] file type is not permitted!', ['filetype' => $this->file->type]));
        }
    ...
    public function getImagesList(MediaModule $module)
    {
        ...
        foreach ($thumbs as $alias => $url) {
            ...
            $list[$url] = Yii::t('art/media', $preset['name']) . ' ' . $preset['size'][0] . ' × ' . $preset['size'][1];
        }

        $originalImageSize = $this->getOriginalImageSize($module->routes);
        $list[$this->url] = Yii::t('art/media', 'Original') . ' ' . $originalImageSize;
     ...
     * @return array|\artsoft\db\ActiveRecord[]
     */
